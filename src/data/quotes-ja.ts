import { Quote } from '../data/quotes';

// Japanese quotes
export const quotesJa: Quote[] = [
  {
    id: 1,
    text: "素晴らしい仕事をする唯一の方法は、あなたがすることを愛することです。",
    author: "スティーブ・ジョブズ",
    category: "成功",
    backgroundGradient: "from-purple-600 via-pink-600 to-blue-600"
  },
  {
    id: 2,
    text: "イノベーションはリーダーとフォロワーを区別します。",
    author: "スティーブ・ジョブズ",
    category: "リーダーシップ",
    backgroundGradient: "from-cyan-500 via-blue-500 to-purple-600"
  },
  {
    id: 3,
    text: "人生とは、あなたが他の計画を立てるのに忙しい間に起こることです。",
    author: "ジョン・レノン",
    category: "人生",
    backgroundGradient: "from-green-400 via-blue-500 to-purple-600"
  },
  {
    id: 4,
    text: "未来は夢の美しさを信じる人々のものです。",
    author: "エレノア・ルーズベルト",
    category: "夢",
    backgroundGradient: "from-pink-500 via-red-500 to-yellow-500"
  },
  {
    id: 5,
    text: "最も暗い瞬間こそ、光を見るために集中しなければならない時です。",
    author: "アリストテレス",
    category: "忍耐",
    backgroundGradient: "from-gray-900 via-purple-900 to-violet-800"
  },
  {
    id: 6,
    text: "始める方法は、話すのをやめて行動を始めることです。",
    author: "ウォルト・ディズニー",
    category: "行動",
    backgroundGradient: "from-orange-400 via-pink-500 to-red-500"
  },
  {
    id: 7,
    text: "昨日が今日の多くを占めることを許してはいけません。",
    author: "ウィル・ロジャース",
    category: "マインドフルネス",
    backgroundGradient: "from-blue-400 via-purple-500 to-pink-500"
  },
  {
    id: 9,
    text: "本当に大切に思っていることに取り組んでいる場合、押される必要はありません。ビジョンがあなたを引っ張ります。",
    author: "スティーブ・ジョブズ",
    category: "情熱",
    backgroundGradient: "from-violet-600 via-purple-600 to-blue-600"
  },
  {
    id: 10,
    text: "世界を変えられると思うほど狂った人々が、実際に変える人々です。",
    author: "ロブ・シルタネン",
    category: "変化",
    backgroundGradient: "from-yellow-400 via-red-500 to-pink-500"
  },
  {
    id: 11,
    text: "あなたができると信じれば、あなたは既に半分成功しています。",
    author: "セオドア・ルーズベルト",
    category: "自信",
    backgroundGradient: "from-indigo-500 via-purple-500 to-pink-500"
  },
  {
    id: 12,
    text: "成功は最終的ではなく、失敗は致命的ではありません：継続する勇気が重要です。",
    author: "ウィンストン・チャーチル",
    category: "勇気",
    backgroundGradient: "from-slate-900 via-purple-900 to-slate-900"
  },
  {
    id: 13,
    text: "唯一不可能な旅は、始めない旅です。",
    author: "トニー・ロビンズ",
    category: "動機",
    backgroundGradient: "from-amber-500 via-orange-500 to-red-500"
  },
  {
    id: 14,
    text: "困難の真ん中に機会があります。",
    author: "アルベルト・アインシュタイン",
    category: "機会",
    backgroundGradient: "from-teal-400 via-blue-500 to-purple-600"
  },
  {
    id: 15,
    text: "私たちの後ろにあるものと前にあるものは、私たちの中にあるものと比べると小さなことです。",
    author: "ラルフ・ワルド・エマーソン",
    category: "内なる力",
    backgroundGradient: "from-rose-400 via-pink-500 to-purple-600"
  },
  {
    id: 16,
    text: "自分らしくいてください；他の人は既に取られています。",
    author: "オスカー・ワイルド",
    category: "真正性",
    backgroundGradient: "from-cyan-400 via-blue-500 to-indigo-600"
  },
  {
    id: 17,
    text: "打たないシュートは100%外します。",
    author: "ウェイン・グレツキー",
    category: "機会",
    backgroundGradient: "from-blue-600 via-purple-600 to-blue-800"
  },
  {
    id: 18,
    text: "木を植える最良の時は20年前でした。次に良い時は今です。",
    author: "中国のことわざ",
    category: "行動",
    backgroundGradient: "from-green-600 via-emerald-600 to-teal-600"
  }
];
